...
export const ReviewForm = ({productId, className, isOpened, ...props}: ReviewFormProps): JSX.Element => {
    ...
    function handleAlertClose(type: 'error' | 'success') {
        return function () {
            switch (type) {
                case 'success':
                    setSuccess(false)
        ...
        <div className={classNames(styles.reviewForm, className)} {...props}>
            ...
            <div className={styles.rating}>
                ...
                <Controller
                    ...
                    render={({field: {value, onChange, ref}}) => <Rating rating={value}
                                                                         ref={ref}
                                                                         tabIndex={isOpened ? 0 : -1}
                                                                         error={errors.rating}
            ...
            {success && <div className={classNames(styles.success, styles.panel)}>
              ...
              <button type='button'
                      onClick={handleAlertClose('success')}
            ...
            {error && <div className={classNames(styles.error, styles.panel)}>
              ...
              <button onClick={handleAlertClose('error')} className={styles.closeIcon}>
                  <CloseIcon/>
              </button>
            </div>}
        </div>
